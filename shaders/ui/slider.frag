...
void main() {
    ...
    vec2 sq_size = vec2(0.475, 0.15) * uv_norm;
    float sq_dist = df_square(nuv-uv_norm*0.5, sq_size);
    float circ_dist = df_circle(nuv-0.5 - vec2(value * sq_size.x*2.0 - 0.2, 0.0), 0.0);


    final_color = fblend(final_color, color, inside(0.0, 0.01, 0.01, sq_dist));
    final_color = fblend(final_color, 1.0-color, bump(0.4, 0.01, 0.01, circ_dist));
    final_color = fblend(final_color, 1.0-color, inside(0.4, 0.00, 0.01, circ_dist)*0.95);
